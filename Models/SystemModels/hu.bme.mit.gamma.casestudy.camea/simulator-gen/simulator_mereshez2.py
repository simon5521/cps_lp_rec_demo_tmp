...
sources = [
		PeriodicEventSource(
name="TrafficSimulator_basic__Traffic_0",
dirac=Dataset(
			dbname=None,
			query=None,
			ip=None,
...
traci.simulationStep(120*sim_speed)
step=0
time=0.0
data=[]
...
"""
		...
		,
		ecalls=[lambda:sctmodel.getTrafficSimulator().getBasic().getInPort().raiseNewCar(),]
		),
]
...
def simulateUntilStop():
	...
	for fault in faults:
		if fault.eventSource.name == "TrafficSimulator_basic__Traffic_0":
			sctmodel.getTrafficSimulator().getBasic().getInPort().raiseNewCar()
